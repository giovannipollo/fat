...
class WeightFaultInjectionHook:
    ...
    def _gradient_hook(self, grad: Tensor) -> Tensor:
        ...
        grad_modified = torch.where(
            self._current_fault_mask, torch.zeros_like(grad), grad
        )
    ...
    def __call__(
        ...
        mask = mask_flat.view(quant_weight.value.shape)
        ...
        if self._grad_hook_handle is None:
            self._grad_hook_handle = module.weight.register_hook(self._gradient_hook)
